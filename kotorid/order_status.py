...
async def _already_resolved(conn: asyncpg.Connection, order_id: str) -> bool:
    val = await conn.fetchval(
        "SELECT 1 FROM alerts "
        "WHERE alert_type IN ('order_filled','order_failed') "
        "  AND symbol IN (SELECT symbol FROM ic_positions WHERE order_id=$1) "
        "LIMIT 1",
        order_id,
    )
...
async def check_open_orders(
    conn: asyncpg.Connection, client: httpx.AsyncClient, account_id: str,
) -> int:
    ...
    for ic in rows:
        ...
        if await _already_resolved(conn, order_id):
            continue
