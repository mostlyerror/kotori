# kotori — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build "kotori" — a Python + Textual TUI portfolio tracker with a persistent background daemon, four switchable views (Inbox, Grid, Kanban, Briefing), and Cockpit-style position detail, optimized for an iron condor earnings strategy alongside directional positions.

**Architecture:** A background daemon (`kotorid`) writes all state to SQLite. The TUI (`kotori_tui`) reads from SQLite every 3s and renders four named views — Inbox [i], Dashboard Grid [g], Kanban [k], Briefing [b] — with a persistent status bar showing NAV, VIX, regime, and alert count. Position detail (Cockpit split-view) activates from any view via ↵.
...
- Daemon architecture: `.worktrees/the-inbox/kotorid/`
- IV/regime engines: `.worktrees/the-watcher/kotorid/jobs.py`
- Kanban view: `.worktrees/the-pipeline/kotori_tui/widgets/kanban_board.py`
- Briefing view: `.worktrees/the-briefing-room/kotori_tui/widgets/briefing_view.py`
- Cockpit detail: `.worktrees/the-cockpit/kotori_tui/widgets/position_detail.py`
...
- Create: `kotorid/__init__.py`
- Create: `kotori_tui/__init__.py`
...
[project.scripts]
kotorid = "kotorid.__main__:main"
kotori = "kotori_tui.__main__:main"

[tool.hatch.build.targets.wheel]
packages = ["kotorid", "kotori_tui"]
...
```python
# kotorid/__init__.py  (empty)
# kotori_tui/__init__.py  (empty)
...
```bash
git add pyproject.toml db/schema.sql kotorid/__init__.py kotori_tui/__init__.py tests/__init__.py .env.example
...
Expected: `ModuleNotFoundError: No module named 'kotorid.db'`

- [ ] **Step 3: Implement `kotorid/db.py`**
...
```bash
git add kotorid/db.py tests/test_db.py
...
Expected: `ModuleNotFoundError: No module named 'kotorid.iv_engine'`

- [ ] **Step 3: Implement `kotorid/iv_engine.py`**
...
```bash
git add kotorid/iv_engine.py tests/test_iv_engine.py
...
**Files:**
- Create: `kotorid/regime_engine.py`
...
Expected: `ModuleNotFoundError: No module named 'kotorid.regime_engine'`

- [ ] **Step 3: Implement `kotorid/regime_engine.py`**
...
```bash
git add kotorid/regime_engine.py tests/test_regime_engine.py
...
**Files:**
- Create: `kotorid/position_monitor.py`
...
- [ ] **Step 4: Implement `kotorid/position_monitor.py`**
...
```bash
git add kotorid/position_monitor.py tests/test_position_monitor.py
...
Expected: `ModuleNotFoundError: No module named 'kotorid.mock_data'`

- [ ] **Step 3: Implement `kotorid/mock_data.py`**
...
```bash
git add kotorid/mock_data.py tests/test_mock_data.py
...
**Files:**
- Create: `kotorid/config.py`
- Create: `kotorid/__main__.py`

- [ ] **Step 1: Implement `kotorid/config.py`**
...
- [ ] **Step 2: Implement `kotorid/__main__.py`**
...
async def run():
    ...
    scheduler.start()
    log.info("kotorid running (TRADIER_ENV=%s)", __import__('kotorid.config', fromlist=['TRADIER_ENV']).TRADIER_ENV)
...
- [ ] **Step 3: Create `kotorid/jobs.py` with stubs**
...
```bash
timeout 3 .venv/bin/kotorid || true
```

Expected: logs show "kotorid running", then exits cleanly after 3s.

- [ ] **Step 5: Commit**

```bash
git add kotorid/config.py kotorid/__main__.py kotorid/jobs.py
...
- [ ] **Step 3: Implement `run_position_monitor` in `kotorid/jobs.py`**
...
```bash
timeout 3 .venv/bin/kotorid 2>&1 | grep -E "running|ERROR"
```

Expected: `kotorid running (TRADIER_ENV=sandbox)` with no ERRORs.

- [ ] **Step 6: Commit**

```bash
git add kotorid/jobs.py
...
**Files:**
- Create: `kotori_tui/__main__.py`
- Create: `kotori_tui/app.py`
- Create: `kotori_tui/db.py`
- Create: `kotori_tui/widgets/__init__.py`
- Create: `kotori_tui/widgets/status_bar.py`
- Create: `kotori_tui/views/__init__.py`
- Create: `kotori_tui/screens/__init__.py`

- [ ] **Step 1: Implement `kotori_tui/db.py`**
...
- [ ] **Step 2: Implement `kotori_tui/widgets/status_bar.py`**
...
- [ ] **Step 3: Implement `kotori_tui/app.py`**
...
class KotoriApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        from kotori_tui.views.inbox_view import InboxView
        from kotori_tui.views.grid_view import GridView
        from kotori_tui.views.kanban_view import KanbanView
...
- [ ] **Step 4: Implement `kotori_tui/__main__.py`**
...
```python
# kotori_tui/views/inbox_view.py
from textual.widget import Widget
from textual.widgets import Label
...
# kotori_tui/views/kanban_view.py
from textual.widget import Widget
from textual.widgets import Label
...
# kotori_tui/views/briefing_view.py
from textual.widget import Widget
from textual.widgets import Label
...
```bash
timeout 3 .venv/bin/kotori 2>&1 | head -5 || true
```
...
**Files:**
- Modify: `kotori_tui/views/inbox_view.py`
...
```bash
git add kotori_tui/views/inbox_view.py
...
**Files:**
- Modify: `kotori_tui/views/grid_view.py`
...
```bash
git add kotori_tui/views/grid_view.py
...
**Files:**
- Modify: `kotori_tui/views/kanban_view.py`
...
```bash
git add kotori_tui/views/kanban_view.py
...
**Files:**
- Modify: `kotori_tui/views/briefing_view.py`
...
```bash
git add kotori_tui/views/briefing_view.py
...
**Files:**
- Create: `kotori_tui/screens/position_detail.py`
- Modify: `kotori_tui/app.py`

- [ ] **Step 1: Implement `kotori_tui/screens/position_detail.py`**
...
- [ ] **Step 2: Register screen in `kotori_tui/app.py`**
...
```bash
git add kotori_tui/screens/position_detail.py kotori_tui/app.py
...
**Files:**
- Create: `kotori_tui/screens/note_input.py`
- Create: `kotori_tui/screens/thesis_editor.py`

- [ ] **Step 1: Implement `kotori_tui/screens/note_input.py`**
...
- [ ] **Step 2: Implement `kotori_tui/screens/thesis_editor.py`**
...
```bash
git add kotori_tui/screens/note_input.py kotori_tui/screens/thesis_editor.py
...
**Files:**
- Modify: `explore.sh`
- Modify: `kotori_tui/app.py` (final cleanup)
...
- [ ] **Step 3: Add kotori to `explore.sh`**
...
TITLES=(
  "★ kotori       — Hybrid: Inbox+Grid+Kanban+Briefing in one app. (RECOMMENDED)"
...
```bash
git add -A
git commit -m "feat: kotori — hybrid TUI with Inbox/Grid/Kanban/Briefing views complete"
```
